...
class CascadeRoIHead(BaseRoIHead):
    ...
    def contrastive(self, stage: int, namuda: List[Tensor], x: Tuple[Tensor], sampling_results: List[SamplingResult], ):
        ...
        for l in range(0, len(labels)):
            criterion = SupConLoss(stage=stage, layer=l, namuda=namuda[0])
            layer_loss = criterion(features, labels)
            cumulative_loss += namuda[1] * layer_loss if l==0 else layer_loss
    ...
    def loss(self, x: Tuple[Tensor], rpn_results_list: InstanceList,
        ...
        losses = dict()
        namuda = torch.tensor([3, 0.6], dtype=torch.float32).to(rpn_results_list[0].bboxes.device)
